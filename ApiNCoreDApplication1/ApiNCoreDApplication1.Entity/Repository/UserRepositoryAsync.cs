...
namespace ApiNCoreDApplication1.Entity.Repository
{
    ...
    public class UserRepositoryAsync : IRepositoryAsync<User>
    {
        ...
        private ApiNCoreDApplication1Context _context;
        public UserRepositoryAsync(ApiNCoreDApplication1Context dbProvider)
        {
        ...
        public async Task Update(object id, User entity)
        {
            if (entity != null)
            {
                string sQuery = "UPDATE Users SET FirstName=@FirstName, LastName=@LastName, UserName=@UserName, Email = @Email, Description=@Description, IsAdminRole=@IsAdminRole, Roles = @Roles, IsActive=@IsActive,Password=@Password,AccountId =@AccountId, Modified=GETUTCDATE() "
                                    + " WHERE Id = @Id";
                if (_transaction == null) Transaction = _connection.BeginTransaction();
